...
# CONFIG
# =========================================================
# Tokenni BotFather'dan olib shu yerga yozing yoki environment orqali bering.
BOT_TOKEN = os.getenv("BOT_TOKEN", "8738414496:AAEvH134tYUmMKqc9Z6vKEfeqodXTSz173Q")
...
PHONE_TEXT = os.getenv("PHONE_TEXT", "+998 99 845 56 51\n+998 94 368 00 06")
...
MIN_DEPOSIT = 1  # amalda 30% ishlatiladi; juda arzon buyurtmalar uchun 0 chiqmasin
...
PRODUCT_CATALOG = {
    "Tort": {
        ...
        "quantity_hint": (
    "1. Tortlar sonini kiriting\n"
    "Masalan: 1 dona, 2 dona\n\n"
    "2. Nechi kishilikligini kiriting\n"
    "Masalan: 2 kishilik, 15 kishilik\n\n"
    "Ko'rinish: 1 dona, 10 kishilik"
    ),
},
    "Pirojniy": {
        "groups": {
            "Oddiy pirojniylar": ["Fistacho", "Maxroviy", "Jiyan", "Ptichi moloko", "Monaco", "Mars"],
            "Mevali pirojniylar": [
                "Yagodniy pirojniy",
                "Qulupnayli pirojniy",
                "Malinali pirojniy",
            ...
            "Rulet va konteyner": ["Mars konteyner", "Makli oq rulet", "Tvarogli rulet", "Tvarogli momiq"],
        },
        "quantity_hint": "Masalan: 1 dona, 6 dona, 12 dona",
    },
    "Trayfel": {
        "groups": {"Trayfel turlari": ["Mitti trayfel", "Kichik trayfel", "Katta trayfel", "Mevali trayfel"]},
        "quantity_hint": "Masalan: 1 dona, 2 dona, 5 dona",
    },
    "Somsa": {
        "groups": {"Go'shtli": ["Go'shtli", "Tovuqli"], "Sabzavotli": ["Oshqovoqli", "Ko'katli", "Kartoshkali"]},
        "quantity_hint": "Masalan: 5 dona, 10 dona, 20 dona",
    },
    "Kruassan": {
        "groups": {"Kruassan turlari": ["Marojnali", "Shokoladli", "Qulupnayli", "Fistashkali"]},
...
def init_db() -> None:
    ...
    ensure_column(cur, "order_items", "design_image", "TEXT")
    ensure_column(cur, "order_items", "design_choice", "TEXT")
...
def design_choice_to_text(choice):
    mapping = {
        1: "Bolalar uchun topper",
        2: "Qizlar uchun topper",
        3: "Chop etilgan rasm",
        4: "Vaflili yeyiladigan rasm",
    }
    try:
        return mapping.get(int(choice), "-")
    except:
        return "-"
...
def auto_deposit_by_total(total_int: int) -> int:
    deposit = max(int(total_int * 0.3), MIN_DEPOSIT)
    return deposit
...
def build_variant_text(category: str) -> str:
    groups = PRODUCT_CATALOG.get(category, {}).get("groups", {})
    lines = [f"<b>{html.escape(category)} bo'limidagi turlar:</b>", ""]
    idx = 1
    for group_name, items in groups.items():
        lines.append(f"<b>{html.escape(group_name.strip())}</b>")
        for item in items:
            item = item.strip()
            if item:
                has_photo = " 🖼" if item in PRODUCT_PHOTOS else ""
                lines.append(f"{idx}. {html.escape(item)}{has_photo}")
                idx += 1
        lines.append("")
...
def build_simple_cart_text(cart: list[dict]) -> str:
    ...
    for i, item in enumerate(cart, start=1):
        lines.append(f"{i}. <b>{html.escape(item.get('product_title', '-'))}</b>")
        lines.append(f"   📏 Miqdori: {html.escape(item.get('quantity_value', '-'))}")
        design_choice = item.get("design_choice")
        design_text = item.get("design_text")
        if design_choice:
            lines.append(f"🎀 Bezagi: {design_choice_to_text(design_choice)}")
        if design_text:
            lines.append(f"📝 Qo'shimcha: {design_text}")
        if item.get("color"):
            lines.append(f"   🎨 Rang: {html.escape(item['color'])}")
        if item.get("top_text"):
            lines.append(f"   ✍️ Yozuv: {html.escape(item['top_text'])}")
        if item.get("filling"):
            lines.append(f"   🍓 Qavat/tam: {html.escape(item['filling'])}")
        if item.get("extra_note"):
            lines.append(f"   📝 Izoh: {html.escape(item['extra_note'])}")
        lines.append("")
    return "\n".join(lines)
...
def selected_product_needs_design(category: str, product_title: str) -> bool:
    return category == "Tort" and product_title not in PREMIUM_CAKES
...
def create_order_from_simple_cart(
    ...
    # 1) avval orders jadvaliga yozamiz
    cur.execute(
        """
        INSERT INTO orders (
    ...
    # 2) keyin order_id ni olamiz
    order_id = cur.lastrowid

    # 3) savatdagi itemlarni order_items ga yozamiz
    for item in cart:
        cur.execute(
            """
            INSERT INTO order_items (
                order_id, product_title, product_category, quantity_value,
                design_text, color, top_text, filling, extra_note, design_image, design_choice
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                order_id,
                ...
                item.get("filling", ""),
                item.get("extra_note", ""),
                item.get("design_image", ""),
                item.get("design_choice", ""),
            ),
        )
...
def build_order_preview(order_id: int) -> str:
    ...
    for idx, item in enumerate(items, start=1):
        lines.append(f"{idx}. <b>{html.escape(item['product_title'])}</b>")
        lines.append(f"   📏 Miqdori: {html.escape(item['quantity_value'])}")
        design_choice = item["design_choice"] if "design_choice" in item.keys() else ""
        design_text = item["design_text"] if "design_text" in item.keys() else ""
        if design_choice:
            lines.append(f"🎀 Bezagi: {html.escape(design_choice_to_text(design_choice))}")
        if design_text:
            lines.append(f"📝 Qo'shimcha bezak: {html.escape(design_text)}")
        if item["color"]:
            lines.append(f"🎨 Rang: {html.escape(item['color'])}")
        if item["top_text"]:
            lines.append(f"✍️ Yozuv: {html.escape(item['top_text'])}")
        ...
        lines.append("")
    if order:
        lines.append(f"👤 Ism: {html.escape(order['full_name'])}")
        lines.append(f"📞 Telefon: {html.escape(order['phone'])}")
        lines.append(f"📅 Sana: {html.escape(order['delivery_date'] or '-')}")
        lines.append(f"⏰ Vaqt: {html.escape(order['delivery_time'] or '-')}")
        lines.append(f"🚚 Yetkazish turi: {html.escape(order['delivery_type'] or '-')}")
        if order["pickup_branch"]:
            lines.append(f"🏬 Filial: {html.escape(order['pickup_branch'])}")
        if order["address"]:
            lines.append(f"📍 Manzil: {html.escape(order['address'])}")
        if order["notes"]:
            lines.append(f"📝 Izoh: {html.escape(order['notes'])}")
        if order["reference_type"]:
            lines.append(f"📌 Namuna turi: {html.escape(order['reference_type'])}")
        if order["custom_description"]:
            lines.append(f"✍️ Qo'shimcha tarif: {html.escape(order['custom_description'])}")
    lines.append("")
    lines.append(f"💬 Narx va zakolat yuborish: <code>/price {order_id} 150000</code>")
    lines.append(f"💬 Yoki depozit bilan: <code>/price {order_id} 150000 45000</code>")
    return "\n".join(lines)
...
def build_admin_stats_text() -> str:
    ...
    def stats_block(title: str, rows: list[sqlite3.Row]) -> str:
        deposits = sum_money_from_rows(
            [r for r in rows if r["status"] in {STATUS_DEPOSIT_SENT, STATUS_CONFIRMED, STATUS_READY, STATUS_DELIVERED}],
            "deposit_amount",
        )
...
def pickup_branch_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🏬 Chortoq filiali")],
            [KeyboardButton(text="🏬 Uychi filiali")],
            [KeyboardButton(text="🔙 Ortga")],
        ],
        resize_keyboard=True,
        one_time_keyboard=True,
...
async def start_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Assalomu alaykum! 👋\n\n"
        f"<b>{html.escape(BRAND_TEXT)}</b> ga xush kelibsiz.\n\n"
        "⏰ <b>Muhim eslatma:</b>\n"
        "Tortlarni kamida <b>2 kun oldin</b> buyurtma bering.\n"
        "Qolgan mahsulotlarni esa odatda <b>1 kun oldin</b> bersangiz ham bo'ladi.\n"
        "Murakkab buyurtmalar vaqtiga qarab oldinroq berish tavsiya etiladi.",
...
async def info_handler(message: Message) -> None:
    await message.answer(
        ...
        "💳 Zakolat buyurtma umumiy narxining <b>30%</b> qismini tashkil qiladi.\n\n"
...
async def contact_handler(message: Message) -> None:
    await message.answer(
        f"📞 Aloqa uchun:\n{html.escape(PHONE_TEXT)}\n\n"
        f"💬 Telegram: {html.escape(TELEGRAM_TEXT)}\n"
        f"🕘 Ish vaqti: {html.escape(WORK_TIME_TEXT)}"
    )
...
async def get_product_variant(message: Message, state: FSMContext) -> None:
    ...
    if not selected_variant:
        await message.answer(f"Iltimos, {html.escape(category)} uchun raqam yoki nomdan birini to'g'ri yuboring.")
        return
    ...
    caption = f"<b>{html.escape(selected_variant)}</b>\n\nQuyidagi tugma orqali shu mahsulotni tanlang."
    if photo_id:
        await message.answer_photo(photo=photo_id, caption=caption, reply_markup=photo_preview_keyboard(selected_variant))
    else:
        await message.answer(
            f"<b>{html.escape(selected_variant)}</b>\n\nBu mahsulot uchun rasm hali qo'shilmagan.\nQuyidagi tugma orqali davom eting.",
...
async def pick_product_callback(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(product_title=product_title, selected_product=product_title)
    await callback.answer("Tanlandi")

    if category == "Tort":
        await callback.message.answer(
            "📏 Tort ma'lumotini kiriting:\n\n"
            "1. Tortlar soni\n"
            "Masalan: 1 dona, 2 dona\n\n"
            "2. Nechi kishilik\n"
            "Masalan: 10 kishilik, 15 kishilik\n\n"
            "Birga yozing:\n"
            "<b>1 dona, 10 kishilik</b>",
...
async def cart_amount(message: Message, state: FSMContext) -> None:
    ...
    if message.text == "🔙 Ortga":
        await state.set_state(CheckoutStates.product_variant)
        await message.answer(
            "Kerakli mahsulot turini yozing yoki raqamini yuboring:",
            reply_markup=only_back_keyboard(),
        )
        return

    await state.update_data(quantity_value=(message.text or "").strip())

    # faqat oddiy tortlarda bezak chiqadi
    if category == "Tort" and product_title not in PREMIUM_CAKES:
        await message.answer(
            "🎀 Bezakni tanlang:\n\n"
            "1️⃣ Bolalar uchun topper\n"
            "2️⃣ Qizlar uchun topper\n"
            "3️⃣ Chop etilgan rasm\n"
            "4️⃣ Vaflili yeyiladigan rasm",
...
async def cart_design(message: Message, state: FSMContext) -> None:
    ...
    if message.text == "🔙 Ortga":
        ...
        if category == "Tort":
            ...
            await message.answer("📏 Miqdorni qayta kiriting", reply_markup=only_back_keyboard())
        return
    ...
    design_choice = int(message.text)
    await state.update_data(design_choice=design_choice)

    if design_choice in [3, 4]:
        await message.answer("📸 Bezak rasmi yuboring:", reply_markup=only_back_keyboard())
        await state.set_state(AddToCartStates.design_image)
        return
...
@dp.message(AddToCartStates.design_image, F.photo)
async def handle_design_photo(message: Message, state: FSMContext) -> None:
    await state.update_data(design_image=message.photo[-1].file_id)

    await message.answer("✅ Bezak rasmi qabul qilindi")
    await message.answer(
        "🎨 Rangini kiriting.\nKerak bo'lmasa 'yo'q' deb yozing.",
        reply_markup=only_back_keyboard(),
    )
...
@dp.message(AddToCartStates.design_image)
async def handle_design_invalid(message: Message, state: FSMContext) -> None:
    if message.text == "🔙 Ortga":
        await state.set_state(AddToCartStates.design)
        await message.answer(
            "🎀 Bezakni tanlang:\n\n"
    ...
    await message.answer("❗ Iltimos, bezak rasmini yuboring.")
...
async def cart_note(message: Message, state: FSMContext) -> None:
    ...
    summary = (
        "<b>Mahsulot tayyor:</b>\n\n"
        f"🍰 Nomi: {html.escape(data.get('product_title', data.get('selected_product', '')))}\n"
        f"📏 Miqdori: {html.escape(data.get('quantity_value', ''))}\n"
        f"🎀 Bezak: {html.escape(data.get('design_text', '') or '-')}\n"        
        f"🎨 Rang: {html.escape(data.get('color', '') or '-')}\n"
        f"✍️ Yozuv: {html.escape(data.get('top_text', '') or '-')}\n"
        f"🍓 Qavat/tam: {html.escape(data.get('filling', '') or '-')}\n"
        f"📝 Izoh: {html.escape(data.get('extra_note', '') or '-')}"
    )
...
async def confirm_add_to_cart(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"✅ <b>{html.escape(cart_item['product_title'])}</b> savatga qo'shildi.\n"
...
async def checkout_delivery_type(message: Message, state: FSMContext) -> None:
    ...
    if message.text == "🚚 Yetkazib berish":
        await message.answer(
            "Hozirda dastavka xizmatimiz yo'qligi sababli online taxi orqali yuboriladi.\n📍 Manzilni kiriting:",
            reply_markup=only_back_keyboard(),
        )
...
async def finalize_checkout(message: Message, state: FSMContext, reference_type: str, custom_description: str, reference_photo_id: str) -> None:
    ...
    preview_text = build_order_preview(order_id)

    for admin_id in ADMIN_IDS:
        if reference_photo_id:
            await bot.send_photo(
                admin_id,
                photo=reference_photo_id,
        ...
        # 3 yoki 4 bezak rasmi bo'lsa, adminlarga alohida yuboriladi
        for idx, item in enumerate(cart, start=1):
            design_choice = item.get("design_choice")
            design_image = item.get("design_image")

            if design_choice in [3, 4] and design_image:
                await bot.send_photo(
                    admin_id,
                    photo=design_image,
                    caption=(
                        f"🖼 Buyurtma #{order_id} uchun bezak rasmi\n"
                        f"🍰 Mahsulot: {html.escape(item.get('product_title', '-'))}\n"
                        f"🎀 Bezagi: {html.escape(design_choice_to_text(design_choice))}\n"
                        f"🔢 Item: #{idx}"
                    ),
                )

    await message.answer("✅ Buyurtmangiz adminga yuborildi. Narx va zakolat kelishini kuting.", reply_markup=main_keyboard())
    await state.clear()
...
async def choose_card_payment(message: Message) -> None:
    ...
    await message.answer(
        f"{CARD_TEXT}\n\n"
        f"💵 Umumiy narx: <b>{html.escape(order['total_price'])}</b>\n"
        f"💳 Tashlanadigan zakolat: <b>{html.escape(order['deposit_amount'])}</b>\n"
        f"💰 Qolgan to'lov: <b>{html.escape(order['remaining_amount'])}</b>\n\n"
...
async def choose_cash_payment(message: Message) -> None:
    ...
    await message.answer(
        ...
        f"💳 Baribir zakolat uchun <b>{html.escape(order['deposit_amount'])}</b> kartaga tashlashingiz kerak.\n"
        f"💰 Qolgan to'lov: <b>{html.escape(order['remaining_amount'])}</b> ni mahsulotni olayotganda berasiz.\n\n"
        f"{CARD_TEXT}\n\n"
        "📸 Endi zakolat chekini yuboring.\n"
        "⛔ Zakolatsiz buyurtma qabul qilinmaydi.",
        reply_markup=copy_card_keyboard(),
...
@dp.message(F.photo)
async def photo_router(message: Message, state: FSMContext) -> None:
    current_state = await state.get_state()

    # 1) Admin mahsulot rasmi yuklayapti
    if current_state == AdminPhotoStates.waiting_photo.state:
        data = await state.get_data()
        product_name = data.get("admin_photo_product", "")

        if not product_name:
            await state.clear()
            await message.answer("Mahsulot topilmadi. /setphoto ni qaytadan bosing.")
            return
        ...
        await state.clear()
        await message.answer(
            f"✅ <b>{html.escape(product_name)}</b> uchun rasm saqlandi.",
            reply_markup=main_keyboard(),
        )
        return

    # 2) Checkoutdagi namuna rasmi
    if current_state == CheckoutStates.reference_photo.state:
        await finalize_checkout(message, state, "photo", "", message.photo[-1].file_id)
        return

    # 3) Mijoz zakolat cheki yuboryapti
    order = get_latest_actionable_order_for_user(message.from_user.id)
    if not order:
        await message.answer("Bu rasm hozir hech qaysi jarayon uchun qabul qilinmadi.")
        return

    if order["status"] not in {STATUS_AWAITING_DEPOSIT_CHECK, STATUS_PRICED}:
        await message.answer("Hozir chek yuborish uchun faol buyurtma topilmadi.")
        return

    deposit_int = safe_int_from_money(order["deposit_amount"] or "")
    if deposit_int is None:
        await message.answer("Zakolat summasi hali to'g'ri belgilanmagan. Admin bilan bog'laning.")
        return
    ...
    for admin_id in ADMIN_IDS:
        await bot.send_photo(
            ...
            caption=(
                ...
                f"💵 Umumiy narx: {html.escape(order['total_price'])}\n"
                f"💳 Zakolat: {html.escape(order['deposit_amount'])}\n"
                f"💰 Qolgan to'lov: {html.escape(order['remaining_amount'])}\n"
                f"💼 Qolgan to'lov usuli: {html.escape(order['payment_type'] or '-')}"
            ),
...
async def set_photo_start(message: Message, state: FSMContext) -> None:
    ...
    if len(parts) < 2:
        await message.answer(
            "To'g'ri format:\n<code>/setphoto Bento</code>\n\n"
            "Masalan:\n"
            "<code>/setphoto Snikersli</code>"
        )
        return
    ...
    await message.answer(
        f"📸 Endi <b>{html.escape(matched)}</b> uchun rasm yuboring.",
        reply_markup=only_back_keyboard(),
    )
...
async def set_photo_waiting_invalid(message: Message, state: FSMContext) -> None:
    if message.text == "🔙 Ortga":
        await state.clear()
...
async def last_orders_handler(message: Message) -> None:
    ...
    for row in rows:
        lines.append(
            f"#{row['id']} | {html.escape(row['full_name'])} | {html.escape(row['phone'])} | "
            f"{html.escape(row['delivery_date'] or '-')} {html.escape(row['delivery_time'] or '-')} | "
            f"{html.escape(row['total_price'] or '-')} | {html.escape(row['deposit_amount'] or '-')} | {html.escape(row['status'])}"
        )
    lines.append("")
    lines.append("💬 Narx va zakolat yuborish: <code>/price ORDER_ID 150000</code>")
    lines.append("💬 Yoki depozit bilan: <code>/price ORDER_ID 150000 45000</code>")
    await message.answer("\n".join(lines))
...
async def admin_order_action_handler(callback: CallbackQuery) -> None:
    ...
    if action == "price":
        ...
        deposit_hint = order["deposit_amount"] or str(auto_deposit_by_total(safe_int_from_money(total_hint) or 150000))
    ...
    await bot.send_message(
        ...
        f"{user_text}\n\n"
        f"🆔 Buyurtma raqami: #{order_id}\n"
        f"💵 Umumiy narx: {html.escape(order['total_price'] or '-')}\n"
        f"💳 Zakolat: {html.escape(order['deposit_amount'] or '-')}\n"
        f"💰 Qolgan to'lov: {html.escape(order['remaining_amount'] or '-')}"
    )
    await callback.answer(label)
    await callback.message.answer(
        f"{label}\n\n"
        f"🆔 Buyurtma: #{order_id}\n"
        f"👤 Mijoz: {html.escape(order['full_name'])}\n"
        f"📌 Holati: {html.escape(new_status)}\n"
        f"💵 Umumiy narx: {html.escape(order['total_price'] or '-')}\n"
        f"💳 Zakolat: {html.escape(order['deposit_amount'] or '-')}\n"
        f"💰 Qolgan to'lov: {html.escape(order['remaining_amount'] or '-')}"
    )
